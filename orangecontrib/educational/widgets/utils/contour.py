...
class Contour:
    ...
    def find_contours(self, threshold):
        ...
        for i in range(bitmap.shape[0] - 1):
            ...
            if sq_idx in [1, 3, 5, 7] and not self.visited(i, 0, (False if sq_idx != 5 else True)):
                contours.append(self.find_contour_path(bitmap, i, 0, threshold))
            # right
            sq_idx = self.corner_idx(bitmap[i:i+2, bitmap.shape[1]-2:bitmap.shape[1]])
            if sq_idx in [4, 5, 12, 13] and not self.visited(i, bitmap.shape[1]-2, False):
        ...
        for j in range(bitmap.shape[1] - 1):
            ...
            if sq_idx in [8, 9, 10, 11] and not self.visited(0, j, (False if sq_idx != 10 else True)):
                contours.append(self.find_contour_path(bitmap, 0, j, threshold))
            # bottom
            sq_idx = self.corner_idx(bitmap[bitmap.shape[0]-2:bitmap.shape[0], j:j+2])
            if sq_idx in [2, 6, 10, 14] and not self.visited(bitmap.shape[0]-2, j, False):
                contours.append(self.find_contour_path(bitmap, bitmap.shape[0]-2, j, threshold))

        nonzero_lines = np.nonzero(bitmap.shape[1] - np.sum(bitmap[1:-1, :], axis=1))[0] + 1
        # 1:-1 to avoid double check edge

        for i in nonzero_lines:
            for j in range(1, bitmap.shape[1] - 2):
                if self.corner_idx(bitmap[i:i+2, j:j+2]) not in [0, 15] and not self.visited(i, j, False):
    ...
    def find_contour_path(self, bitmap, start_i, start_j, threshold):
        i, j = start_i, start_j
        path = [self.to_real_coordinate(self.start_point(bitmap[i:i+2, j:j+2], np.array([i, j]), threshold))]
        ...
        while 0 <= i < bitmap.shape[0] - 1 \
            ...
            new_p = self.new_point(square, previous_position, np.array([i, j]), threshold)
            path.append(self.to_real_coordinate(new_p))

            self.mark_visited(i, j, (True if (self.corner_idx(square) in [5, 10] and
                                              (previous_position is None  # if none algorithm always select upper path
                                               or previous_position[0] < i or previous_position[1] < j)) else False))
            previous_position_tmp = [i, j]

            i, j = self.new_position(square, previous_position, np.array([i, j])).tolist()
    ...
    def new_point(self, sq, previous, position, threshold):
        ...
        if con_idx == 5:
            goes_top = ((previous is None and position[1] != self.z.shape[1] - 2)
            ...
            goes_right = ((previous is None and position[0] != self.z.shape[0] - 2) or
    ...
    def start_point(self, sq, position, threshold):
        ...
        if con_idx == 5:
            ...
            pos = position + np.array([self.triangulate(threshold, heat_from, heat_to),
            ...
            pos = position + np.array([0 if from_top else 1, self.triangulate(threshold, heat_from, heat_to)])
